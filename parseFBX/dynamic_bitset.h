...
class dynamic_bitset_base
{
		...
		m_num_blocks(calc_num_blocks(num_bits))
	{
		if (m_num_blocks != 0)
		{
			m_bits = reinterpret_cast<Block*> (malloc(m_num_blocks * sizeof(Block)));
...
inline dynamic_bitset::dynamic_bitset(size_type num_bits, unsigned long value)
  : dynamic_bitset_base(num_bits)
{
	const size_type M = sizeof(unsigned long) * 8 > num_bits ? num_bits : sizeof(unsigned long) * 8;
...
inline void dynamic_bitset::resize(size_type num_bits, bool value)
{
	...
	Block* d = reinterpret_cast<Block*> (malloc(sizeof(Block) * new_nblocks));
